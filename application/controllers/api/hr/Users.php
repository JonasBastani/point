...
class Users extends CI_Controller
{
    ...
    public function list()
    {
        ...
        // removendo os usuários administrador da listagem
        $this->db->where('profile_id <>', 3);

        // verificando parâmetros e formando query
        if (!empty($profileId)) {
        ...
        // buscando usuários
        $users = $this->users_my_model->get_all();
        if (!empty($users)) {
            $this->api->repponse(200, ['status' => true, 'data' => $users, 'Tudo certo!']);
        } else {
            $this->api->repponse(200, ['status' => true, 'data' => '', 'Nenhum usuário encontrado!']);
        }
    }
}
